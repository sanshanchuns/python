...
# training data
x = np.linspace(-7, 10, N_SAMPLES)[:, np.newaxis]  #(2000, 1)
np.random.shuffle(x)
noise = np.random.normal(0, 2, x.shape)
y = np.square(x) - 5 + noise
train_data = np.hstack((x, y))  #(2000, 2)

# test data
test_x = np.linspace(-7, 10, 200)[:, np.newaxis]  #(200, 1)
noise = np.random.normal(0, 2, test_x.shape)
test_y = np.square(test_x) - 5 + noise  #(200, 1)
